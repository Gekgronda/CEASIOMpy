...
def add_new_aeromap(tixi, aeromap, aeromap_uid, aeromap_name):
    ...
    NEW_AEROMAP_PATH = f"{AEROPERFORMANCE_XPATH}/aeroMap[@uID='{aeromap_uid}']"
    ...
    if not tixi.checkElement(NEW_AEROMAP_PATH):
        tixi.createElement(AEROPERFORMANCE_XPATH, "aeroMap")
        tixi.addTextAttribute(f"{AEROPERFORMANCE_XPATH}/aeroMap[last()]", "uID", aeromap_uid)
    ...
    create_or_update_element(
     ...
     # Ensure aeroPerformanceMap exists
    performance_map_path = f"{NEW_AEROMAP_PATH}/aeroPerformanceMap"
    if not tixi.checkElement(performance_map_path):
        tixi.createElement(NEW_AEROMAP_PATH, "aeroPerformanceMap")
...
def avl_update(tixi, aeromap_name, avl_params):
    ...
    # Ensure toolsoecific exists
    if not tixi.checkElement("/cpacs/toolspecific"):
        tixi.createElement("/cpacs", "toolspecific")
    
    # Ensure CEASIOMPY exists
    if not tixi.checkElement(CEASIOMPY_XPATH):
        tixi.createElement("/cpacs/toolspecific", "CEASIOMpy")
    

    # Ensure aerodynamics exists
    if not tixi.checkElement("/cpacs/vehicles/aircraft/model/analyses/aerodynamics"):
        tixi.createElement("/cpacs/vehicles/aircraft/model/analyses", "aerodynamics")
    
    # Create aerodynamics
    if not tixi.checkElement("/cpacs/toolspecific/CEASIOMpy/aerodynamics"):
