...
def data_gen(n):
    ...
    altitude = np.random.randint(0, 15000, n)  # Altitudine random tra 0 e 15000 metri
    mach = np.round(np.random.uniform(0.1, 0.6, n), 1)  # Mach number tra 0.1 e 0.6 con 1 decimale
    aoa = np.random.randint(-4, 15, n)  # Angolo di attacco tra -4 e 15 gradi
    aos = np.random.randint(-15, 15, n)  # Angolo di derapata tra -15 e 15 gradi
...
# Chiedi all'utente il numero di predizioni da generare
n = int(input("Insert the number of predictions: "))
