...
def extract_coefficients(base_path, forces_breakdown_name):
    """Estrae CL, CD, CSF, CMx, CMy, e CMz dai file forces_breakdown.dat nelle sottocartelle."""
    results = []
    ...
    for i, directory in enumerate(directories):
        ...
        file_path = os.path.join(directory_path, forces_breakdown_name)
...
def append_to_new_csv(data, original_filename):
    """Crea un nuovo CSV con i dati estratti e lo salva con suffisso '_TRAIN'."""
...
if __name__ == "__main__":
    base_path = (
        ...
        or "/wrk/Gronda/labAR/EULER/takeoff0"
    )
    csv_filename = (
        input("Inserisci percorso del dataset da aggiornare: ").strip()
        or "/wrk/Gronda/labAR/EULER/takeoff0/updated_takeoff0.csv"
    )
    fb_name = (
        input("Inserisci il nome del forces_breakdown file: ").strip() or "forces_breakdown.dat"
    )

    if os.path.isdir(base_path):
        data = extract_coefficients(base_path, fb_name)
