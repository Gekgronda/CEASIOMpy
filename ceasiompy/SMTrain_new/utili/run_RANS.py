...
# Definire i percorsi
principal_path = "/wrk/Gronda/RANS"
original_config_path = "/wrk/Gronda/RANS/mesh_120/Config_labAR_jack.cfg"
dataset_path = "/wrk/Gronda/RANS/calculator.csv"

# Leggere il dataset saltando le prime 3 righe
data = pd.read_csv(dataset_path, skiprows=4)
...
for index, row in data.iterrows():
    # Aggiornare l'indice per allinearlo alla numerazione Case3 in poi
    case_index = index + 3

    # 1. Creare la cartella Case{n}
    case_dir = os.path.join(principal_path, f"Case{case_index}")
    os.makedirs(case_dir, exist_ok=True)

    # 2. Copiare il file config nella cartella
    config_path = os.path.join(case_dir, "Config_labAR_jack.cfg")
    ...
    for line in lines:
        if line.startswith("MACH_NUMBER="):
        ...
        elif line.startswith("FREESTREAM_DENSITY ="):
            line = f"FREESTREAM_DENSITY ={row['density']}\n"
            ...
            line = f"REYNOLDS_NUMBER={row['reynolds_number']}\n"
    ...
    if process.returncode != 0:
        print(f"Errore durante l'esecuzione di SU2 per Case{case_index}")
        break
